...
const fetchYouTubeData = async (url) => {
  try {
    ...
    const response = await fetch('/api/youtube-analyzer', {
      ...
      body: JSON.stringify({ url, type: isChannel ? 'channel' : 'video' })
    });
...
const fetchAnalysisHistory = async (user) => {
  try {
    ...
    if (userId) {
      return allHistoryData.filter(item => item.userId === userId);
    }
...
function YouTubeCommentsAnalyzer() {
  ...
  const loadAnalysisFromHistory = async (historyItem) => {
    ...
    try {
      const data = await fetchYouTubeData(historyItem.url);
      setAnalysisData(data);
      
      // Load the saved QA history if available
      if (historyItem.qaHistory && Array.isArray(historyItem.qaHistory)) {
  ...
  return (
    <div className="container mx-auto py-6 px-2.5 max-w-7xl">
          ...
          {isLoadingHistory ? (
            ...
            <div className="space-y-2">
              {history.map((item, index) => (
                <button
                  key={index}
                  className={`flex w-full items-center justify-between border p-3 rounded-md hover:bg-gray-50 transition-colors text-left ${item.id === currentAnalysisId ? 'bg-blue-50 border-blue-200' : ''}`}
                ...
                >
                  <div className="flex items-center gap-4">
                    <img
                      ...
                      className="w-12 h-12 object-cover rounded-md overflow-hidden"
                    />
                    <div>
      ...
      <div className="text-center text-xs text-gray-500 mt-8">
        © 2025 YT Comments Analyzer. All rights reserved.
      </div>
    </div>
  );
}
